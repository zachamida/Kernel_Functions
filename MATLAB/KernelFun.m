...
switch KernelType
    case 'RBF' % RBF kernel including (ARD: Automatic Relevance Determination)
        ...
        K=exp(-0.5*(K));
    case 'AAK' % Aitchison and Aitken (1976) unordered discrete kernel
        h=Param;
    ...
    case 'Anisotropic-RBF' % Anisotropic RBF kernel
        ...
        x = x*D;
        y = y*D;
        K = KernelFun(x,y,1,'RBF');
    case 'Laplace' % Laplace kernel
        K=zeros(N,M);
        for i=1:length(Param)
            K=K+sqrt(CalculateDistance(x(:,i),y(:,i)),0)./(2*Param(i)^2);
        end
        K=exp(-K);
